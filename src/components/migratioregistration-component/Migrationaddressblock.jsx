...
// Same cascading Province -> District -> Local Level -> Ward pattern as
// WardOfficeInfo / birth's AddressInfo, but this writes straight into
// formData.addresses[index] for whichever addressType it's rendering
// (PERMANENT / CURRENT / NEW) instead of resolving a separate
// register_ward_id. Also mirrors AddressInfo's read-only Nepali display
// fields (province_np/district_np/municipality_np/ward_name_np), filled
// in from the matching ward row as each select is made.
function MigrationAddressBlock({
  addressType,
  title,
  subtitle,
  wards = [],
  ...
  // Each level reset clears the Nepali fields below it too, and fills in
  // this level's own Nepali value from any matching ward row — same
  // pattern as birth's AddressInfo.jsx.
...
export default MigrationAddressBlock;
