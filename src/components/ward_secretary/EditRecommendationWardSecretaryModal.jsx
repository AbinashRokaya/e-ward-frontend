...
// Same static-file convention as EditRecommendationModal.jsx (data
// validation stage) — LEGACY paths come back as
...
function EditRecommendationWardSecretaryModal({
  wards,
  recommendation,
  onClose,
  onSaved,
  ...
  const handleVerified = (e) => {
    e.preventDefault();
    fetch(
      `${API_URL}/v1/ward-secretary/recommendation/${formData.letter_id}/approve`,
      {
        method: "POST",
        credentials: "include",
      ...
      .then(() => {
        if (onSaved) onSaved();
        onClose();
        toast.success("Recommendation letter verified successfully!");
      })
      .catch((err) => {
        console.error("Verify failed:", err);
        toast.error(err?.detail || "Verify failed. Please try again.");
      });
  };
  ...
  const handleRejected = (e) => {
    ...
    fetch(
      `${API_URL}/v1/ward-secretary/recommendation/${formData.letter_id}/reject`,
      {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
...
export default EditRecommendationWardSecretaryModal;
