...
function EditMigrationRegistrationWardChairpersonModal({
  wards,
  migration,
  onClose,
  onSaved,
}) {
  const [formData, setFormData] = useState({ ...migration });
  ...
  const handleVerified = (e) => {
    e.preventDefault();
    fetch(
      `${API_URL}/v1/ward-chairperson/migration/${formData.migration_id}/approve`,
      {
        method: "POST",
        credentials: "include",
      ...
      .then(() => {
        if (onSaved) onSaved();
        onClose();
        toast.success("Migration registration approved successfully!");
      })
  ...
  const handleRejected = (e) => {
    ...
    fetch(
      `${API_URL}/v1/ward-chairperson/migration/${formData.migration_id}/reject`,
      {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
      ...
      .then(() => {
        if (onSaved) onSaved();
        onClose();
        toast.success("Migration registration rejected successfully!");
      })
...
export default EditMigrationRegistrationWardChairpersonModal;
