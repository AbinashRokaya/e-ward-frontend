...
function EditBirthRegistrationWardChairpersonModal({
  wards,
  birth,
  onClose,
  onSaved,
  ...
  const handleVerified = (e) => {
    e.preventDefault();
    fetch(
      `${API_URL}/v1/ward-chairperson/${formData.registration_id}/approve`,
      {
        method: "POST",
        credentials: "include",
  ...
  const handleRejected = (e) => {
    ...
    fetch(`${API_URL}/v1/ward-secretary/${formData.registration_id}/reject`, {
      ...
      .then(() => {
        if (onSaved) onSaved();
        onClose();
        toast.success("Birth registration rejected successfully!");
      })
...
export default EditBirthRegistrationWardChairpersonModal;
